...
### Steel solver efficiency

The solver retains all 128 rigidity iterations. Reciprocal links, rest lengths and correction weights are resolved once per physics step and reused across iterations. CPU scratch arrays are reused and allocated only when solid steel exists. WebGPU compacts solid steel indices into an indirect work list: iterations visit only those particles, while other material state is copied once during preparation. Fully molten/erased steel gives a zero-sized dispatch. The cache rebuilds after bond updates each step, so cuts, new bonds and phase changes cannot reuse stale links.

This trades approximately 12.5 MiB of GPU cache storage at full capacity for less repeated work. `node tests/steel-benchmark.cjs` compares CPU projection time and exact final coordinates against the pre-cache implementation (git commit `5b193b5`). A 400-particle test measured about 1.6× faster projection here; this is solver timing, not an end-to-end frame-rate claim. Native GPU checks cover the compact work list, unchanged non-steel state and zero-work molten worlds.

Solid steel turns visibly red at the 250°C combustible-ignition threshold in every view, with stepped red highlights and shadows. It returns to gray below that threshold; molten steel remains yellow-hot. This early red tint is an intentional gameplay hazard cue, distinct from the physical-incandescence approximation used for the higher-temperature halo.

## Clone

**Clone** is an anchored, ice-like solid: gravity, explosions, turbines, lights and the player cannot move it, heat does not melt it, and the eraser removes it. A freshly painted clone is blank (dim violet). The first material that touches it programs it, and it turns bright violet. A blank clone that touches a programmed clone copies its program, so a whole connected body learns from one contact. Life, absorbed cargo and seeds carry genomes that do not fit in the stored program, so they do not program a clone.

A programmed clone produces its material at a steady rate: each clone tries once every 24 physics steps (10 times per second at normal speed). It places the new particle 0.8 cells away, in the first free spot of four rotating directions. A clone surrounded by particles, including other clones, produces nothing, so only the exposed surface of a clone body emits. Cloned molten steel and mercury vapor start hot enough to stay in that phase.
