...
(async()=>{
 ...
 let cookie='',profileName='Player',current=world(),running=true,restores=0,dropResponse=true,saveIds=[];
 ...
 w.fetch=async(route,init)=>{
   ...
   if(route==='/api/showcase/projects'&&init.method==='POST'){saveIds.push(JSON.parse(init.body).requestId);if(dropResponse){dropResponse=false;await response.text();throw new Error('Test: response lost');}}
   return response;
 };
 ...
 try{
   ...
   const name=d.getElementById('character-name');name.value='TRANNY';name.dispatchEvent(new w.Event('change'));assert.equal(name.value,'♥♥♥');
   ...
   name.value='Mage';name.dispatchEvent(new w.Event('change'));click('Showcase');await wait(()=>d.querySelector('.project-card'));click('Fork');await wait(()=>restores===2&&!d.querySelector('dialog').open);assert.equal(profileName,'Mage');assert(d.getElementById('project-context').textContent.startsWith('Fork of'));
   click('Save project');await wait(()=>d.querySelector('form'));assert(d.querySelector('form').textContent.includes('original project'));assert.equal(d.querySelector('form input').value,'Garden remix');click('Close');
   w.dispatchEvent(new w.Event('slurry-clear'));assert.equal(d.getElementById('project-context').textContent,'');
   console.log('PASS showcase DOM flow, named character, modal pause/resume, retry after lost response, previews, own-vote control, load confirmation and fork attribution');
