...
{
const fs=require('fs'),assert=require('node:assert/strict'),src=fs.readFileSync(require('node:path').join(__dirname,'..','slurry','index.html'),'utf8').split('<script>')[1].split('// ---------- WGSL ----------')[0],m={exports:{}};new Function('module',src+'\nmodule.exports={Engine,makeParams,Stickman};')(m);const {Engine,makeParams,Stickman}=m.exports;
