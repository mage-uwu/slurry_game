...
p.reset();for(let yy=0;yy<56;yy++)e.wall[yy*80+50]=1;step(350);p.keys.add('KeyD');step(300);assert(p.x<=24.16);console.log('PASS solid-wall collision and no tunneling');
const e2=new Engine(40,28,2000,makeParams({jitter:0})),p2=new Stickman(40,28);for(let xx=1;xx<39;xx+=.55)for(let yy=24;yy<27;yy+=.55)e2.add(xx,yy,3);for(let k=0;k<400;k++){e2.step();p2.step(e2);}assert(p2.y<27&&p2.y>20);console.log('PASS stands on granular terrain',p2.y);
// Synthetic stationary fluid bed isolates swimming controls from a draining pool.
const e3=new Engine(40,28,2000,makeParams({gy:0,jitter:0})),p3=new Stickman(40,28);p3.x=20;p3.y=18;for(let yy=8;yy<23;yy+=.5)for(let xx=16;xx<25;xx+=.5)e3.add(xx,yy,1);e3.sort();p3.keys.add('KeyW');for(let k=0;k<20;k++)p3.step(e3);assert(p3.wet&&p3.y<17.7);p3.keys.clear();p3.keys.add('KeyS');for(let k=0;k<50;k++)p3.step(e3);assert(p3.y>18);console.log('PASS swim up and dive down');
