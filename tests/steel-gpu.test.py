...
this.result={shaders:{steel:SH_STEEL,bonds:SH_BONDS,relax:SH_RELAX,tiled:SH_RELAX_T,density:SH_DENSITY,densityT:SH_DENSITY_T,count:SH_COUNT,move:SH_BONDMOVE,player:SH_PLAYER,dots:SH_DOTS},n:e.n,initial,attrs:Array.from(e.attr.slice(0,e.n)),pid:Array.from(e.pid.slice(0,e.n)),bonds:Array.from(e.bonds.slice(0,e.n*8)),idx:Array.from(e.idx.slice(0,200)),starts:Array.from(e.cellStart)};
...
for k in range(32):p.set_bind_group(0,bgs[k%2]);p.dispatch_workgroups((n+255)//256)
...
print('PASS steel GPU shader compilation and 32-iteration strained-block projection matching CPU')
