(() => {
  function bootShowcase() {
    ...
    const bar=el('div',{id:'project-bar'}),character=el('input',{id:'character-name',maxlength:'24','aria-label':'Character name',placeholder:'Player',autocomplete:'off'});
    ...
    bar.append(context,button('Save project',()=>open('save'),'primary'),button('Showcase',()=>open('gallery')));
    ...
    let profile=null,remaining=4,parent=null,currentTitle='',resume=false,busy=false,generation=0,retrySave=null;
    ...
    function updateCharacter(){character.value=format.name(character.value,24,'Player');try{localStorage.setItem('slurry-character',character.value);}catch{}retrySave=null;}
    ...
    async function open(next) {
      if(busy)return;
      resume=bridge.pause();dialog.showModal();content.replaceChildren(status);heading.textContent=next==='save'?'Save to showcase':'Community showcase';say('Loading…');
    ...
    function showSave() {
      ...
      const characterLabel=el('label',{},'Character name');characterLabel.append(character);
      const note=el('p',{},'Your world will be public. Anyone can load it, give it a thumbs-up, or save their own version.');
      ...
      form.append(titleLabel,characterLabel,note,quota,submit);content.replaceChildren(form,status);say('');
      ...
      form.onsubmit=async event=>{
        ...
        try {
          bridge.setName(character.value);
          await ensureProfile();
          ...
          form.append(button('Browse showcase',()=>{heading.textContent='Community showcase';gallery();}));
    ...
    async function load(project) {
      ...
      try{
        ...
        parent=project.id;currentTitle=project.title;
        context.textContent='Loaded · '+project.title;resume=false;dialog.close();
      ...
      async function page(reset){
        ...
        try{
          ...
          for(const project of data.projects){
            ...
            if(project.parent)body.append(button('↳ Original',async()=>{try{const data=await api('/projects/'+project.parent);await load(data.project);}catch(error){say(error.message,true);}}));
            ...
            actions.append(like,button('Load',()=>load(project)));body.append(actions);card.append(canvas,body);grid.append(card);
          }
