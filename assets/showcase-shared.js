...
(() => {
  ...
  function preview(arrays,s) {
    ...
    for(let i=0;i<s.count;i++){const x=Math.floor(arrays.state[i*4+2]),y=Math.floor(arrays.state[i*4+3]);if(x>=0&&x<160&&y>=0&&y<100)pixels[y*160+x]=arrays.attributes[i]&31;}
  ...
  function validate(s) {
    ...
    for (const v of arrays.attributes) if ((v & 31)>20) throw new Error('Unknown material.');
